 ...
 * “#ifdef _MYSTICAL”中,提供了POSIX并不包含的扩展部分。其中有些扩展的意义是很明显的。
 * 比如定义57,600或以上的波特率,以及对终端显示窗口的支持。正如任何合理的标准都是开放
 * 的那样,POSIX标准并不排斥扩展。但在MYSTICAL环境下想写一个可移植到其他环境的程序时,就
 * 必须注意避免使用那些局限于MYSTICAL的特征。这一点很容易做到。在该文件和其他定义面向
 * MYSTICAL的扩展的文件中,这些扩展是通过以下语句控制的。
 * # ifdef _MYSTICAL
 * 如果 _MYSTICAL未被定义,编译器将根本不会感知到MYSTICAL扩展。
 */

#ifndef MYSTICAL_TERMIOS_H
#define MYSTICAL_TERMIOS_H
